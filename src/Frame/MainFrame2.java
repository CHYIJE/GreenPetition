...
import ver1.ObjectDAO.LoginDAO2;
...
public class MainFrame2 extends JFrame {
	...
	private String id;
	private String name;
	private static LoginDAO2 mcontext;

	private int num;
	private String stnum;
	private int max;

	public MainFrame2(LoginDAO2 loginDAO2) {
		this.mcontext = loginDAO2;
//		numbersheet();
//		addAction();
		try {
			initData();
	...
	public List<PatitionDTO> petitionTable() throws SQLException{
		String header[] = {"제목","유저", "게시일"};
		model = new DefaultTableModel(header, 0);
		table = new JTable(model);

		List<PatitionDTO> list = new ArrayList<>();
		String viewQuery = " SELECT id, title, user_id, date, agree, disagree FROM petition"; 
		try (Connection conn = DBConnectionManager.getInstance().getConnection()){
			PreparedStatement viewptmt = conn.prepareStatement(viewQuery);
			ResultSet rs = viewptmt.executeQuery();
...
//			while(rs.next()) {
//				PatitionDTO dto = new PatitionDTO().builder()
//						.id(rs.getInt("id"))
